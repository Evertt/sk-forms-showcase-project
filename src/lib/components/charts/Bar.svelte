<script lang="ts">
	...
	// Had to add the if-statements to prevent the error:
	// `<rect> attribute width: A negative value is not valid.`.
	// Because when the viewport is small enough,
	// for the dashboard to get replaced by a PNG,
	// the width and height variables becomes zero.

	$: if (width) xScale = scaleLinear()
		.domain([0, xTicks.length])
		.range([padding.left, width - padding.right, 0]);

	$: if (height) yScale = scaleLinear()
		.domain([0, Math.max.apply(null, yTicks)])
		.range([height - padding.bottom, padding.top]);

	$: if (width) innerWidth = width - (padding.left + padding.right);
